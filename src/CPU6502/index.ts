...
export default class CPU6502 implements IDevice {
  ...
  private* _sty() {
    const address = yield* this._addressingMode();
    yield* this._write(address, this._y);
    yield* this._stall(4);
  }
  ...
  private* _tay() {
    ...
    this._setNegativeFlag((this._y & 0x80) === 0x80);
    this._setZeroFlag(this._y === 0);
  ...
  private* _tax() {
    ...
    this._setNegativeFlag((this._x & 0x80) === 0x80);
    this._setZeroFlag(this._x === 0);
